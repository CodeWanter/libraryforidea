...
<head>
    ...
    <script type="text/javascript">
	    ...
	    $(document).ready(function() { 
	        ...
	        var marcPath = '${marcPath}';
	        $.ajax({
	             type: "Post",
	             url: "${path }/publish/loadData",
	             data: {batchCode:batchCode, accessPath:accessPath, marcPath:marcPath},
	             dataType: "json",
	             beforeSend: function () {
	             	load();
	             },
	             ...
	             success: function(data){
	             	var opts = {
             			 href : '${path }/publish/dataMerge?batchCode='+batchCode,
                         title : '数据合并',
                         border : false,
                         closable : true,
                         fit : true,
	    ...
	    function load() {  
	        ...
	        $("<div class=\"datagrid-mask-msg\"></div>").html("数据解析加载中，请稍候。。。").appendTo("body").css({ display: "block", left: ($(document.body).outerWidth(true) - 190) / 2, top: ($(window).height() - 45) / 2 });  
	    }
	    //取消加载层  
	    function disLoad() {  
    ...
    <div id="mainLayout" class="easyui-layout" data-options="fit:true, border:false">
        <div data-options="region:'north',border:false, collapsedSize:0" style="height:50px;">
            <div class="head">
                <table>
                    <tr>
                        <td width="50%" style="font-size: 14px;">国图云平台</td>
