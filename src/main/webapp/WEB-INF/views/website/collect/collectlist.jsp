...
<div class="LS2018_main">
    ...
    <div class="LS2018_bd Z_clearfix">
        <div class="LS2018_MBX">
            当前位置：&nbsp;<a href="${staticPath}/">首 页</a><span class="gt">&gt;</span>每日更新
        </div>
        ...
        <div class="LS2018_Aright">
            <div class="LS2018_module2">
                ...
                <div class="m_middle">
                    <ul class="LS2018_list1" id="daily">
...
$(function () {
    ...
    $.post(basePath + '/forehead/article/topTenData',{"title":"","nowpage":1,"pageSize":5,"sort":"createTime","order":"desc"},function(result){
        result = eval('(' + result + ')');
        var data = result.rows;
        $.each(result.rows, function (i, item) {
            var html = "";
            html += '<li><a  href="${path }/forehead/article/detail?id='+ item.id +'">' + item.title + '</a></li>';
            $("#news").append(html);
        });
    });
});
...
function paginationCallback(pageIndex, jq) {
    ...
    $.ajax({
        ...
        data: {"title":title, "sort":"collectTime","order":"desc", "pageIndex": pageIndex+1, "pageSize": pageSize},
        dataType: "json",
        async: true,
        beforeSend: function () {
            //index = layer.load(1);
